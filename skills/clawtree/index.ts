...
export async function run(
  ...
  if (query.startsWith("/tree")) {
    ...
    switch (cmd) {
      ...
      case "branch": {
        ...
        // /tree branch list
        if (sub === "list") {
          const branches = treeManager.listBranches();
          if (!branches.length) {
            context.log("[CLAWTREE] No custom branches defined.\n  Add one with: /tree branch add <json>");
            context.log("  Or drop a .json file in <baseDir>/branches/");
          } else {
            context.log(`\n🌱 Custom branches (${branches.length}):\n`);
            for (const b of branches) {
              context.log(`  ${b.label}  (${b.nodes.length} node(s))  key: '${b.name}'`);
            }
          }
          break;
        }

        // /tree branch add <json-string>
        if (sub === "add") {
          const raw = args.slice(3).join(" ");
          if (!raw) {
            context.log([
              "[CLAWTREE] Usage: /tree branch add <json>",
              "  Example:",
              "  /tree branch add {\"name\":\"crypto\",\"label\":\"💹 Crypto\",\"nodes\":[{\"slug\":\"binance-api\",\"name\":\"Binance API\",\"description\":\"Spot & futures data\"}]}",
            ].join("\n"));
            break;
          }
          let def: any;
          try { def = JSON.parse(raw); }
          catch { context.log("[CLAWTREE] ✗ Invalid JSON in branch definition"); break; }
          const errors = treeManager.addBranch(def);
          if (errors.length) {
            context.log("[CLAWTREE] ✗ Validation errors:\n  " + errors.join("\n  "));
          } else {
            context.log(`[CLAWTREE] ✅ Branch '${def.name}' added — run /tree show to see it`);
          }
          break;
        }

        // /tree branch remove <name>
        if (sub === "remove") {
          const name = args[3];
          if (!name) { context.log("[CLAWTREE] Usage: /tree branch remove <branch-name>"); break; }
          const result = treeManager.removeBranch(name);
          if (result.ok) {
            context.log(`[CLAWTREE] 🗑️  Branch '${name}' removed`);
          } else {
            context.log(`[CLAWTREE] ✗ ${result.reason}`);
          }
          break;
        }

        context.log("[CLAWTREE] /tree branch: list | add <json> | remove <name>");
        break;
      }

      default:
        context.log("[CLAWTREE] /tree: show | recommend | install <slug> | evolve <slug> | mode auto|manual|hybrid | branch list|add|remove");
    }
    return { success: true };
  }
  ...
  if (query.startsWith("/fleet")) {
    ...
    switch (cmd) {
      case "status":     printFleetStatus(); break;
      case "sync": {
        context.log("[CLAWTREE] 🔄 Syncing fleet...");
        const status = syncFleet();
        context.log(`\n🌳 Fleet Sync Complete\n  Agents: ${status.agents} | XP: ${status.totalXP} | Installed: ${status.installed}`);
        break;
      }
      case "pull": {
      ...
      case "push": {
        ...
        context.log(`\n⬆️  Fleet Push: ${upgraded} fleet node(s) updated`);
        break;
      }
      case "register": {
        const [,,name,agentPath] = args;
        if (!name || !agentPath) { context.log("Usage: /fleet register <name> <path>"); break; }
      ...
      case "unregister": {
        const name = args[2];
        if (!name) { context.log("Usage: /fleet unregister <name>"); break; }
        const ok = unregisterAgent(name);
        context.log(ok ? `✓ Agent '${name}' removed` : `Agent '${name}' not found`);
        break;
      }
      case "list": {
    ...
    switch (cmd) {
      case "list": {
        ...
        for (const s of slots) {
          ...
          context.log(`  ${status} [${s.id}] ${s.slug} — ${s.trigger} → ${s.action}  (fired ${s.fireCount}x)`);
        }
        break;
      }
      case "add": {
        ...
        if (!slug || !trigger || !action) {
          context.log("Usage: /slots add <slug> <trigger> <action>\n  Triggers: on_use | on_install | on_evolve | on_chain | *\n  Actions: notify_user | auto_install | unlock_bonus_branch | flush_memory | emit_event");
          break;
        }
      ...
      case "remove": {
        const id = args[2];
        if (!id) { context.log("Usage: /slots remove <slot-id>"); break; }
        saveSlots(baseDir, removeSlot(slots, id));
        context.log(`[SLOTS] ✓ Slot '${id}' removed`);
        break;
      }
      case "fire": {
        const [,,eventType,slug] = args;
        if (!eventType || !slug) { context.log("Usage: /slots fire <event-type> <slug>"); break; }
        const results = processEvent({ type: eventType as any, slug, ts: new Date().toISOString() }, tree, baseDir);
        if (!results.length) context.log(`[SLOTS] No slots matched: ${eventType}:${slug}`);
        break;
      }
      default:
